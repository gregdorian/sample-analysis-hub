...
export default function SettingsSampleTypes() {
  const [sampleTypes, setSampleTypes] = useState<SampleType[]>([]);
  const [form, setForm] = useState<Omit<SampleType, "id">>({ nombre: "" });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId !== null) {
      setSampleTypes(items => items.map(i => i.id === editingId ? { ...i, ...form } : i));
      setEditingId(null);
    } else {
      setSampleTypes(items => [...items, { ...form, id: Date.now() }]);
    }
    setForm({ nombre: "" });
  };

  const handleEdit = (id: number) => {
    const item = sampleTypes.find(i => i.id === id);
    if (item) setForm({ nombre: item.nombre });
    setEditingId(id);
  };

  const handleDelete = (id: number) => setSampleTypes(items => items.filter(i => i.id !== id));

  return (
    <Card>
      <CardHeader>
        <CardTitle>Tipos de Muestra</CardTitle>
      </CardHeader>
