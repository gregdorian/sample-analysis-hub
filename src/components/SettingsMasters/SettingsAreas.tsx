...
export default function SettingsAreas() {
  const [areas, setAreas] = useState<Area[]>([]);
  const [form, setForm] = useState<Omit<Area, "id">>({ nombre: "" });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId !== null) {
      setAreas(items => items.map(i => i.id === editingId ? { ...i, ...form } : i));
      setEditingId(null);
    } else {
      setAreas(items => [...items, { ...form, id: Date.now() }]);
    }
    setForm({ nombre: "" });
  };

  const handleEdit = (id: number) => {
    const item = areas.find(i => i.id === id);
    if (item) setForm({ nombre: item.nombre });
    setEditingId(id);
  };

  const handleDelete = (id: number) => setAreas(items => items.filter(i => i.id !== id));
